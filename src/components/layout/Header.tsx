...
import { navigation, NavigationItem } from "./Sidebar";
...
const baseNavigation: { name: string; href: string; id: string; icon: React.ReactNode }[] = [];

navigation.forEach(item => {
    if (item.href) {
        baseNavigation.push({ name: item.name, href: item.href, id: item.id || '', icon: item.icon });
    }
    if (item.children) {
        item.children.forEach(child => {
            if (child.href) {
                baseNavigation.push({ name: child.name, href: child.href, id: child.id || '', icon: child.icon || item.icon });
            }
        });
    }
});
