...
export default function NuevaVentaPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([cajasData, clientesData, productosData]) => {
        ...
        setCajas(allCajas.filter((c: Caja) => c.activo));
        const allClientesRes = Array.isArray(clientesData) ? clientesData : clientesData?.data || [];
        setAllClientes(allClientesRes);
        setClientes(allClientesRes.filter((c: Cliente) => c.activo));
        const prods = Array.isArray(productosData)
          ? productosData
          : productosData?.data || [];
  ...
  const setCantidadDirecta = (idx: number, value: string) => {
    ...
    if (!isNaN(parsed) && parsed >= 1) {
      setLineas((prev) =>
        prev.map((l, i) => (i === idx ? { ...l, cantidad: parsed } : l)),
      );
    }
  };
  ...
  const handleCreateCliente = async (e: React.FormEvent) => {
    ...
    // Validate if client exists globally (ignoring active/inactive)
    const exists = allClientes.find(c => 
      c.identificacion.toLowerCase() === clienteForm.identificacion.toLowerCase() || 
      c.descripcion.toLowerCase() === clienteForm.descripcion.toLowerCase()
    );
    if (exists) {
      setClienteError("El cliente que intenta crear ya existe.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(d.message || "Error al crear cliente");
      }
      
      const newCliente = await res.json();
      
      setAllClientes(prev => [...prev, newCliente]);
      if (newCliente.activo) {
        setClientes(prev => [...prev, newCliente]);
      }
      
      setClienteId(String(newCliente.id));
  ...
  const handleSubmit = async () => {
    if (!fecha || !clienteId || !cajaId) {
      showError(
        "Campos obligatorios",
        "Fecha, cliente y caja son obligatorios.",
      );
      return;
    }
    ...
    for (const l of lineas) {
      if (!l.cantidad || l.cantidad <= 0) {
        showError(
          "Cantidad inválida",
          `La cantidad de "${l.productoDescripcion}" debe ser mayor a 0.`,
        );
        return;
      }
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <div className="flex gap-2 items-center">
              <select
                ...
                className="flex-1 rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-900 transition-all focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
              >
              ...
              <button
                ...
                className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-lg border border-emerald-300 bg-emerald-50 text-emerald-600 transition-colors hover:bg-emerald-100 dark:border-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400 dark:hover:bg-emerald-900/50"
                title="Nuevo Cliente"
              >
          ...
          <div>
            ...
            <div className="relative">
              <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-sm text-gray-400 font-medium">
                $
              </span>
              <input
                type="number"
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {lineas.length > 0 ? (
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            ...
                            value={l.cantidad}
                            onChange={(e) =>
                              setCantidadDirecta(idx, e.target.value)
                            }
            ...
            >
              <svg
                className="mx-auto h-10 w-10 text-gray-300 dark:text-gray-600 mb-3 group-hover:text-emerald-400 dark:group-hover:text-emerald-600 transition-colors"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                Hacé clic en{" "}
                <span className="font-semibold">Agregar producto</span> para
                comenzar
              </p>
            </div>
          )}
        ...
        <div className="flex items-center gap-3">
          {lineas.length > 0 && (
            <span className="text-sm text-gray-500 dark:text-gray-400">
              ...
              unidades · {lineas.length} producto
              {lineas.length !== 1 ? "s" : ""}
            </span>
          )}
        </div>
      ...
      {showModal && (
        ...
        >
          <div
            className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden"
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              ...
              {modalSearch && (
                <p className="mt-2 text-xs text-gray-400">
                  {filteredProductos.length} resultado
                  {filteredProductos.length !== 1 ? "s" : ""} para &quot;
                  {modalSearch}&quot;
                </p>
              )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredProductos.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center px-6">
                  <svg
                    className="h-12 w-12 text-gray-200 dark:text-gray-700 mb-3"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                    Intentá con otro nombre o código
                  </p>
                </div>
              ) : (
                <ul className="divide-y divide-gray-100 dark:divide-gray-800">
                  {filteredProductos.map((p) => {
                    const isSelected = !!lineas.find(
                      (l) => l.productoId === p.id,
                    );
                    return (
                      <li key={p.id}>
            ...
            <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between gap-3 bg-gray-50 dark:bg-gray-800/50">
              <span className="text-xs text-gray-400">
                {filteredProductos.length} de {productos.length} producto
      ...
      {/* ─── Modal Crear Cliente ─── */}
      {showClientModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm overflow-y-auto" onClick={(e) => { if (e.target === e.currentTarget) setShowClientModal(false); }}>
          <div className="w-full max-w-2xl rounded-2xl border border-gray-100 bg-white p-8 shadow-2xl dark:border-gray-800 dark:bg-gray-900">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Nuevo Cliente</h2>
              <button onClick={() => setShowClientModal(false)} className="text-gray-400 transition-colors hover:text-gray-500">
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>

            {clienteError && <div className="mb-4 flex items-center gap-2 rounded-lg bg-red-50 p-4 text-sm text-red-600 dark:bg-red-900/20 dark:text-red-400">
            ...
            <form onSubmit={handleCreateCliente} className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">Descripción / Nombre completo <span className="text-red-500">*</span></label>
                <input required placeholder="Ej: Juan Pérez" value={clienteForm.descripcion} onChange={(e) => setClienteForm({ ...clienteForm, descripcion: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500 transition-all" />
              </div>

              <div>
                <label className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">Tipo Identificación <span className="text-red-500">*</span></label>
                <select value={clienteForm.tipoIdentificacion} onChange={(e) => setClienteForm({ ...clienteForm, tipoIdentificacion: e.target.value as TipoIdentificacion })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 transition-all">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">Identificación <span className="text-red-500">*</span></label>
                <input required placeholder="Ej: 20-12345678-9" value={clienteForm.identificacion} onChange={(e) => setClienteForm({ ...clienteForm, identificacion: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500 transition-all" />
              </div>

              <div>
                <label className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">Teléfono (Opcional)</label>
                <input type="tel" placeholder="Ej: +54 9..." value={clienteForm.telefono} onChange={(e) => setClienteForm({ ...clienteForm, telefono: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500 transition-all" />
              </div>

              <div>
                <label className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">Email (Opcional)</label>
                <input type="email" placeholder="Ej: correo@ejemplo.com" value={clienteForm.email} onChange={(e) => setClienteForm({ ...clienteForm, email: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500 transition-all" />
              </div>

              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-300">Domicilio (Opcional)</label>
                <input placeholder="Ej: Av. Principal 123" value={clienteForm.domicilio} onChange={(e) => setClienteForm({ ...clienteForm, domicilio: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500 transition-all" />
              </div>

              <div className="flex items-center gap-2">
                <input type="checkbox" id="activoCliente" checked={clienteForm.activo} onChange={(e) => setClienteForm({ ...clienteForm, activo: e.target.checked })} className="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" />
                <label htmlFor="activoCliente" className="text-sm font-medium text-gray-700 dark:text-gray-300">Activo</label>
              </div>

              <div className="mt-2 flex justify-end gap-3 border-t border-gray-100 pt-4 dark:border-gray-800 sm:col-span-2">
                <button type="button" onClick={() => setShowClientModal(false)} className="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800">Cancelar</button>
                <button type="submit" disabled={savingCliente} className="rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/20 transition-all hover:bg-emerald-700 active:scale-95 disabled:opacity-50">
