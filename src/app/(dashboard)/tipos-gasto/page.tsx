...
export default function TiposGastoPage() {
  ...
  const fetchTipos = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/tipos-gasto`, { headers: headers() });
      if (!res.ok) throw new Error("Error al cargar tipos de gastos");
      setTipos(await res.json());
    } catch {
      setError("Error al cargar tipos de gastos");
    } finally {
      setLoading(false);
    }
  }, [headers]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || "Error al crear tipo de gasto");
      }
      await fetchTipos();
      closeModal();
      showSuccess("Tipo de gasto creado", "El tipo de gasto ha sido creado correctamente");
    } catch (err: any) {
      showError("Error al crear tipo de gasto", err.message);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`${API_URL}/tipos-gasto/${selected.id}`, {
        method: "PATCH",
        headers: headers(),
        body: JSON.stringify(form),
      });
      if (!res.ok) {
        ...
        throw new Error(d.message || "Error al editar tipo de gasto");
      }
      await fetchTipos();
      closeModal();
      showSuccess("Tipo de gasto actualizado", "El tipo de gasto ha sido actualizado correctamente");
    } catch (err: any) {
      showError("Error al editar tipo de gasto", err.message);
      setError(err.message);
    } finally {
      setSaving(false);
    }
  ...
  const handleToggleStatus = async (tipo: TipoGasto) => {
    const confirmed = await showConfirm({
      title: `¿${tipo.activo ? 'Desactivar' : 'Activar'} tipo de gasto?`,
    ...
    setOpenDropdownId(null);
    try {
      const res = await fetch(`${API_URL}/tipos-gasto/${tipo.id}`, {
        method: "PATCH",
        headers: headers(),
      ...
      await fetchTipos();
      showSuccess("Estado actualizado", `El tipo de gasto ahora está ${!tipo.activo ? 'activo' : 'inactivo'}`);
    } catch (err: any) {
  ...
  const handleExportExcel = () => {
    if (sortedTipos.length === 0) {
      showError("Sin datos", "No hay tipos de gastos para exportar");
      return;
    }
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Tipos de Gastos");

    XLSX.writeFile(workbook, "tipos_de_gastos.xlsx");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Tipos de gastos</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Total de tipos de gastos: {totalItems}</p>
        </div>
          ...
          {paginatedTipos.length === 0 && (
            <div className="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-gray-800/50">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="font-medium text-gray-600 dark:text-gray-300">No se encontraron tipos de gastos</p>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-2xl dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
            ...
            <form onSubmit={modal === "create" ? handleCreate : handleEdit} className="space-y-5">
              <div>
                ...
                <input
                  required
                  placeholder="Ej: Combustible, Librería, Limpieza..."
