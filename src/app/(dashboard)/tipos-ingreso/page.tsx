...
export default function TiposIngresoPage() {
  ...
  const [selected, setSelected] = useState<TipoIngreso | null>(null);
  ...
  const fetchTipos = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/tipos-ingreso`, { headers: headers() });
      if (!res.ok) throw new Error("Error al cargar tipos de ingresos");
  ...
  const openEdit = (tipo: TipoIngreso) => {
    setSelected(tipo);
    setForm({ descripcion: tipo.descripcion, activo: tipo.activo });
    setError("");
    setModal("edit");
    setOpenDropdownId(null);
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`${API_URL}/tipos-ingreso`, {
        method: "POST",
        headers: headers(),
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || "Error al crear tipo de ingreso");
      }
      await fetchTipos();
      closeModal();
      showSuccess("Tipo de ingreso creado", "El tipo de ingreso ha sido creado correctamente");
    } catch (err: any) {
      showError("Error al crear tipo de ingreso", err.message);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`${API_URL}/tipos-ingreso/${selected.id}`, {
        method: "PATCH",
        headers: headers(),
        body: JSON.stringify(form),
      });
      if (!res.ok) {
        ...
        throw new Error(d.message || "Error al editar tipo de ingreso");
      }
      await fetchTipos();
      closeModal();
      showSuccess("Tipo de ingreso actualizado", "El tipo de ingreso ha sido actualizado correctamente");
    } catch (err: any) {
      showError("Error al editar tipo de ingreso", err.message);
      setError(err.message);
    } finally {
      setSaving(false);
    }
  ...
  const handleToggleStatus = async (tipo: TipoIngreso) => {
    const confirmed = await showConfirm({
      title: `¿${tipo.activo ? 'Desactivar' : 'Activar'} tipo de ingreso?`,
      text: `¿Estás seguro de que deseas ${tipo.activo ? 'desactivar' : 'activar'} "${tipo.descripcion}"?`,
      confirmButtonText: tipo.activo ? "Sí, desactivar" : "Sí, activar",
      confirmButtonColor: tipo.activo ? "#ef4444" : "#10b981",
    });
    ...
    setOpenDropdownId(null);
    try {
      const res = await fetch(`${API_URL}/tipos-ingreso/${tipo.id}`, {
        method: "PATCH",
        headers: headers(),
      ...
      await fetchTipos();
      showSuccess("Estado actualizado", `El tipo de ingreso ahora está ${!tipo.activo ? 'activo' : 'inactivo'}`);
    } catch (err: any) {
  ...
  const filteredTipos = tipos.filter((tipo) => {
    const matchesSearch = tipo.descripcion.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === "Todos" ||
      (statusFilter === "Activo" ? tipo.activo : !tipo.activo);
  ...
  const totalItems = sortedTipos.length;
  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * pageSize;
  const paginatedTipos = sortedTipos.slice(startIndex, startIndex + pageSize);
  ...
  const handleExportExcel = () => {
    if (sortedTipos.length === 0) {
      showError("Sin datos", "No hay tipos de ingresos para exportar");
      return;
    }

    const dataToExport = sortedTipos.map((u) => ({
      ID: u.id,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Tipos de Ingresos");

    XLSX.writeFile(workbook, "tipos_de_ingresos.xlsx");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Tipos de ingresos</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Total de tipos de ingresos: {totalItems}</p>
        </div>
              ...
              <div key={tipo.id} className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className={`text-sm font-medium ${tipo.activo ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400 line-through'}`}>
                      {tipo.descripcion}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      Creado: {formatDate(tipo.createdAt)}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      Actualizado: {formatDate(tipo.updatedAt)}
                    </div>
                  </div>
                  <div className="relative">
                    {canEdit && (
                      <button
                        onClick={(e) => { e.stopPropagation(); toggleDropdown(tipo.id); }}
                    ...
                    {openDropdownId === tipo.id && canEdit && (
                      <div ref={mobileDropdownRef} className="absolute right-0 z-50 mt-1 w-48 origin-top-right rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-800 dark:ring-gray-700">
                        <div className="py-1">
                          <button onClick={() => openEdit(tipo)} className="group flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700">
                          ...
                          <button className={`group flex w-full items-center px-4 py-2 text-sm ${tipo.activo ? "text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20" : "text-green-600 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-900/20"}`} onClick={() => handleToggleStatus(tipo)}>
                            <svg className={`mr-3 h-5 w-5 ${tipo.activo ? "text-red-400" : "text-green-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              {tipo.activo ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />}
                            </svg>
                ...
                <div className="mt-2">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ring-1 ring-inset ${tipo.activo
                    ...
                    }`}>{tipo.activo ? "Activo" : "Inactivo"}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="hidden md:block">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-gray-900">
                {paginatedTipos.map((tipo) => (
                  <tr key={tipo.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="whitespace-nowrap px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${tipo.activo
                        ? "bg-green-50 text-green-700 ring-green-600/20 dark:bg-green-900/20 dark:text-green-400 dark:ring-green-400/30"
                        : "bg-red-50 text-red-700 ring-red-600/20 dark:bg-red-900/20 dark:text-red-400 dark:ring-red-400/30"
                        }`}>
                          ...
                          {openDropdownId === tipo.id && (
                            <div ref={dropdownRef} className="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-800 dark:ring-gray-700">
                              <div className="py-1">
                                ...
                                <div className="border-t border-gray-100 dark:border-gray-700 my-1"></div>
                                <button className={`group flex w-full items-center px-4 py-2 text-sm ${tipo.activo ? "text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20" : "text-green-600 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-900/20"}`} onClick={() => handleToggleStatus(tipo)}>
                                  <svg className={`mr-3 h-5 w-5 ${tipo.activo ? "text-red-400 group-hover:text-red-500" : "text-green-400 group-hover:text-green-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    {tipo.activo ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />}
                                  </svg>
                                  {tipo.activo ? "Desactivar" : "Activar"}
                                </button>
          ...
          {paginatedTipos.length === 0 && (
            <div className="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-gray-800/50">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="font-medium text-gray-600 dark:text-gray-300">No se encontraron tipos de ingresos</p>
                <p className="text-gray-400">Intenta con otros filtros o crea un nuevo tipo.</p>
              </div>
            </div>
          )}
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-2xl dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                {modal === "create" ? "Nuevo Tipo de Ingreso" : "Editar Tipo de Ingreso"}
              </h2>
              <button
                onClick={closeModal}
            ...
            <form onSubmit={modal === "create" ? handleCreate : handleEdit} className="space-y-5">
              <div>
                ...
                <input
                  required
                  placeholder="Ej: Ventas, Servicios, Intereses..."
